...
# 조건 2: 일주일 동안의 소정 근로일에 모두 출근 (지각 조퇴 상관 X) -> (현재 매장 룰에 맞춰 생략됨)
...
def calculate_monthly_salary(employee, year, month):
    ...
    raw_total_salary = (total_hours * hourly_wage) + weekly_bonus
    
    before_tax = math.floor(raw_total_salary / 10) * 10
    tax_amount = math.floor((before_tax * 0.033) / 10) * 10
    after_tax = before_tax - tax_amount

    # 일주일 당 발생하는 주휴수당
    holiday_map = get_weekly_holiday_map(employee, start_date, end_date)
    formatted_holiday_map = {
        date_key.strftime('%Y-%m-%d'): float(amount)
        for date_key, amount in holiday_map.items()
    }
    ...
    return {
        ...
        "after_tax": after_tax,  
        "holiday_map_json": json.dumps(formatted_holiday_map, cls=DjangoJSONEncoder),
    }